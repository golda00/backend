...
def generate_vector_report(
    ...
    if callout_records:
        CALLOUT_FONT_SIZE = 12          # Increased for better visibility
    ...
    _draw_legend_stack(
        page, survey_image_path, title_box_data,
...
def _draw_legend_stack(
    ...
    **kwargs,                      # Catch extra args like img_gray, callouts, dpi
    ...
    raw_lines = [
        ...
        f"INST: {inst}"    if inst     else "",
        map_type           if map_type else "",
        f"PG 1 OF {pg_count}",
    ]
    ...
    if survey_image_path:
        # Make the screenshot width at least 450, or larger than the text box by 20%
        target_ss_w = max(450.0, box_w * 1.2)
        try:
